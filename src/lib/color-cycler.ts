export class Color  {
    ...
    private hexVal: string = '#000000';
    private strVal: string = 'rgb(0, 0, 0)';
    private rgbVal: number[] = [0, 0, 0];
    private rgbaVal: number[] = [0, 0, 0, 1];
    private invVal: number[] = [0, 0, 0];
    private pBrightVal: number = 0;
  
    private rVal: number = 0;
    private gVal: number = 0;
    private bVal: number = 0;
    private aVal: number = 0;
...
export class ColorsCycler {
    private index: number;
    private colors: Color[];
    constructor(colors: string[]) {
        this.index = 0;
        this.colors = colors.map(c =>  Color.fromString(c));
    }

    public next(speed:number,deltaT:number):Color  {
        const distance = speed * deltaT;
        const colorIndex = Math.floor(distance) % this.colors.length;
        const colorOffset = (distance - (colorIndex * this.colors.length)) / this.colors.length;
        const c1 = this.colors[colorIndex];
        const c2 = this.colors[(colorIndex+1) % this.colors.length];
        const r = c1.r + (c2.r - c1.r) * colorOffset;
        const g = c1.g + (c2.g - c1.g) * colorOffset;
        const b = c1.b + (c2.b - c1.b) * colorOffset;
        const a = c1.a + (c2.a - c1.a) * colorOffset;
